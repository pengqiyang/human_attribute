...
class ResNet(nn.Module):
    ...
    def forward(self, x):  
        ...
        #pdb.set_trace()
        x = self.layer1(x)
        spatial_att1 = self.transformer_1(x.permute(0, 2, 3, 1).view(x.size()[0], x.size()[2]*x.size()[3], 64))              
       	x = x + spatial_att1.transpose(2, 1).view(x.size()[0], 64, x.size()[2], x.size()[3])
        #print(x.permute(0, 2, 3, 1).view(x.size()[0], x.size()[2]*x.size()[3], 64)[0,:,0] == x[0,0,:,:].view(-1))
...
def resnet18_vit_v3(pretrained=False, progress=True, **kwargs):
