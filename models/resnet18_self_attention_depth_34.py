...
__all__ = ['resnet18_self_attention_depth_34']
att = 0
model_urls = {
...
class SELayer_v2(nn.Module):
    ...
    def forward(self, x, pre):
        ...
        inter = x * y.expand_as(x)
        
        y = self.avg_pool_zi(pre).view(b_pre, c_pre)
       
        y = self.fc_1_zi(y)
        y = self.re_1_zi(y)
        y = self.fc_2_zi(y)
        y = self.re_2_zi(y)
        y = self.si_zi(y).view(b, c, 1, 1)        
        result = inter * y.expand_as(inter)
        return result
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.layer1_d = self._make_layer(block, 64, self.layer_depth[0])
        self.layer2_d = self._make_layer(block, 128, self.layer_depth[1], stride=2,
                                       dilate=replace_stride_with_dilation[0])
        self.layer3_d = self._make_layer(block, 256, self.layer_depth[2], stride=2,
                                       dilate=replace_stride_with_dilation[1])
        self.layer4_d = self._make_layer(block, 512, self.layer_depth[3], stride=2,
                                       dilate=replace_stride_with_dilation[2])
    ...
    def forward(self, x, depth):
        ...
        x_pre = self.att_rgb(x)
       
        
        x = self.layer1(x_pre)
        depth = self.layer1_d(depth) 
        x_pre = self.att_rgb_layer1(x, x_pre)

         
        x = self.layer2(x_pre)
        depth_pre = self.layer2_d(depth) 
        x_pre =  self.att_rgb_layer2(x, x_pre)

        
        x = self.layer3(x_pre)
        depth_1 = self.layer3_d(depth_pre) 
        x_pre = self.att_d_layer3(depth_1, depth_pre) + self.att_rgb_layer3(x, x_pre) #merge


        x = self.layer4(x_pre)
        depth_2 = self.layer4_d(depth_1)     
        x = self.att_d_layer4(depth_2, depth_1) + self.att_rgb_layer4(x, x_pre) #merge        
...
def resnet18_self_attention_depth_34(pretrained=False, progress=True, **kwargs):
