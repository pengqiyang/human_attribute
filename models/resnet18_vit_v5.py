...
class ResNet(nn.Module):
    ...
    def forward(self, x):  
        ...
        x = self.layer1(x)
        #x = self.conv2(x) 
        spatial_att2 = self.transformer_1(x.permute(0, 2, 3, 1).view(x.size()[0], x.size()[2]*x.size()[3], 64))
...
def resnet18_vit_v5(pretrained=False, progress=True, **kwargs):
