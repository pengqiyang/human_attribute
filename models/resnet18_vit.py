...
class ResNet(nn.Module):
    ...
    def forward(self, x):  
        ...
        x = self.layer1(x)
        #spatial_att1 = self.transformer_1(att1.view(x.size()[0], x.size()[2]*x.size()[3], 64))              
        #x = x + spatial_att1.permute(0,2,1).view(x.size()[0],x.size()[1],x.size()[2],x.size()[3])		
      
        x = self.layer2(x)
        spatial_att2 = self.transformer_2(att2.view(x.size()[0], x.size()[2]*x.size()[3], 128))  
        #pdb.set_trace()        
        x = x + spatial_att2.permute(0,2,1).view(x.size()[0],x.size()[1],x.size()[2],x.size()[3])
       
        x = self.layer3(x)
        spatial_att3 = self.transformer_3(att3.view(x.size()[0], x.size()[2]*x.size()[3], 256))     
        x = x + spatial_att3.permute(0,2,1).view(x.size()[0],x.size()[1],x.size()[2],x.size()[3])
...
def resnet18_vit(pretrained=False, progress=True, **kwargs):
