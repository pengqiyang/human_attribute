...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.inception_5a_1x1 = nn.Conv2d(512, 256, kernel_size=(1, 1), stride=(1, 1))
        self.inception_5a_1x1_bn = nn.BatchNorm2d(256, affine=True)
        self.inception_5a_relu_1x1 = nn.ReLU(inplace)
        
        self.inception_5a_3x3_reduce = nn.Conv2d(512, 256, kernel_size=(1, 1), stride=(1, 1))
        self.inception_5a_3x3_reduce_bn = nn.BatchNorm2d(256, affine=True)
        self.inception_5a_relu_3x3_reduce = nn.ReLU(inplace)
        self.inception_5a_3x3 = nn.Conv2d(256, 256, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        self.inception_5a_3x3_bn = nn.BatchNorm2d(256, affine=True)
        self.inception_5a_relu_3x3 = nn.ReLU(inplace)
        
        self.inception_5a_double_3x3_reduce = nn.Conv2d(512, 256, kernel_size=(1, 1), stride=(1, 1))
        self.inception_5a_double_3x3_reduce_bn = nn.BatchNorm2d(256, affine=True)
        self.inception_5a_relu_double_3x3_reduce = nn.ReLU(inplace)
        self.inception_5a_double_3x3_1 = nn.Conv2d(256, 256, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        self.inception_5a_double_3x3_1_bn = nn.BatchNorm2d(256, affine=True)
        self.inception_5a_relu_double_3x3_1 = nn.ReLU(inplace)
        self.inception_5a_double_3x3_2 = nn.Conv2d(256, 256, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        ...
        self.inception_5a_pool_proj = nn.Conv2d(512, 256, kernel_size=(1, 1), stride=(1, 1))
        self.inception_5a_pool_proj_bn = nn.BatchNorm2d(256, affine=True)
        self.inception_5a_relu_pool_proj = nn.ReLU(inplace)
        self.inception_5c_pool = nn.MaxPool2d((3, 3), stride=(2, 2), dilation=(1, 1), ceil_mode=True)       
        
                      
        self.inception_6a_1x1 = nn.Conv2d(1024, 512, kernel_size=(1, 1), stride=(1, 1))
        self.inception_6a_1x1_bn = nn.BatchNorm2d(512, affine=True)
        self.inception_6a_relu_1x1 = nn.ReLU(inplace)
        
        
        self.inception_6a_3x3_reduce = nn.Conv2d(1024, 512, kernel_size=(1, 1), stride=(1, 1))
        self.inception_6a_3x3_reduce_bn = nn.BatchNorm2d(512, affine=True)
        self.inception_6a_relu_3x3_reduce = nn.ReLU(inplace)
        self.inception_6a_3x3 = nn.Conv2d(512, 512, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        self.inception_6a_3x3_bn = nn.BatchNorm2d(512, affine=True)
        self.inception_6a_relu_3x3 = nn.ReLU(inplace)
        
        self.inception_6a_double_3x3_reduce = nn.Conv2d(1024, 512, kernel_size=(1, 1), stride=(1, 1))
        self.inception_6a_double_3x3_reduce_bn = nn.BatchNorm2d(512, affine=True)
        self.inception_6a_relu_double_3x3_reduce = nn.ReLU(inplace)
        self.inception_6a_double_3x3_1 = nn.Conv2d(512, 512, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        self.inception_6a_double_3x3_1_bn = nn.BatchNorm2d(512, affine=True)
        self.inception_6a_relu_double_3x3_1 = nn.ReLU(inplace)
        self.inception_6a_double_3x3_2 = nn.Conv2d(512, 512, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        self.inception_6a_double_3x3_2_bn = nn.BatchNorm2d(512, affine=True)
        self.inception_6a_relu_double_3x3_2 = nn.ReLU(inplace)
        
        
        self.inception_6a_pool = nn.AvgPool2d(3, stride=1, padding=1, ceil_mode=True, count_include_pad=True)
        self.inception_6a_pool_proj = nn.Conv2d(1024, 512, kernel_size=(1, 1), stride=(1, 1))
        self.inception_6a_pool_proj_bn = nn.BatchNorm2d(512, affine=True)
        self.inception_6a_relu_pool_proj = nn.ReLU(inplace)        
        self.inception_6e_pool = nn.MaxPool2d((3, 3), stride=(2, 2), dilation=(1, 1), ceil_mode=True)
    ...
    def forward(self, x, depth):
        ...
        #layer 1
        x = self.layer1(x)   
        conv2_3x3_reduce_out = self.conv2_3x3_reduce(depth)
        conv2_3x3_reduce_bn_out = self.conv2_3x3_reduce_bn(conv2_3x3_reduce_out)
        depth = self.conv2_relu_3x3_reduce(conv2_3x3_reduce_bn_out)       
        x = self.att_rgb_layer1(x)
       
       
        inception_3a_1x1_out = self.inception_3a_1x1(depth)
        inception_3a_1x1_bn_out = self.inception_3a_1x1_bn(inception_3a_1x1_out)
        inception_3a_relu_1x1_out = self.inception_3a_relu_1x1(inception_3a_1x1_bn_out)
        inception_3a_3x3_reduce_out = self.inception_3a_3x3_reduce(depth)
        ...
        inception_3a_double_3x3_reduce_out = self.inception_3a_double_3x3_reduce(depth)
        ...
        inception_3a_pool_out = self.inception_3a_pool(depth)
        ...
        depth = self.inception_3c_pool(inception_3a_output_out)  
       
       
         
        #laye2 
        x = self.layer2(x)
        conv2_3x3_out = self.conv2_3x3(depth)
        conv2_3x3_bn_out = self.conv2_3x3_bn(conv2_3x3_out)
        conv2_relu_3x3_out = self.conv2_relu_3x3(conv2_3x3_bn_out)
        depth = self.pool2_3x3_s2(conv2_relu_3x3_out)        
        x =  self.att_rgb_layer2(x)
        
        inception_4a_1x1_out = self.inception_4a_1x1(depth)
        inception_4a_1x1_bn_out = self.inception_4a_1x1_bn(inception_4a_1x1_out)
        inception_4a_relu_1x1_out = self.inception_4a_relu_1x1(inception_4a_1x1_bn_out)
        inception_4a_3x3_reduce_out = self.inception_4a_3x3_reduce(depth)
        ...
        inception_4a_double_3x3_reduce_out = self.inception_4a_double_3x3_reduce(depth)
        ...
        inception_4a_pool_out = self.inception_4a_pool(depth)
        ...
        depth = self.inception_4e_pool(inception_4a_output_out)        
        

        #layer3
        x = self.layer3(x)    
        inception_5a_1x1_out = self.inception_5a_1x1(depth)
        inception_5a_1x1_bn_out = self.inception_5a_1x1_bn(inception_5a_1x1_out)
        inception_5a_relu_1x1_out = self.inception_5a_relu_1x1(inception_5a_1x1_bn_out)
        inception_5a_3x3_reduce_out = self.inception_5a_3x3_reduce(depth)
        ...
        inception_5a_double_3x3_reduce_out = self.inception_5a_double_3x3_reduce(depth)
        ...
        inception_5a_pool_out = self.inception_5a_pool(depth)
        inception_5a_pool_proj_out = self.inception_5a_pool_proj(inception_5a_pool_out)
        inception_5a_pool_proj_bn_out = self.inception_5a_pool_proj_bn(inception_5a_pool_proj_out)
        inception_5a_relu_pool_proj_out = self.inception_3a_relu_pool_proj(inception_5a_pool_proj_bn_out)
        ...
        depth = self.inception_5c_pool(inception_5a_output_out)
        ...
        #layer4   
        x = self.layer4(x)
        inception_6a_1x1_out = self.inception_6a_1x1(depth)
        inception_6a_1x1_bn_out = self.inception_6a_1x1_bn(inception_6a_1x1_out)
        inception_6a_relu_1x1_out = self.inception_6a_relu_1x1(inception_6a_1x1_bn_out)
        inception_6a_3x3_reduce_out = self.inception_6a_3x3_reduce(depth)
        inception_6a_3x3_reduce_bn_out = self.inception_6a_3x3_reduce_bn(inception_6a_3x3_reduce_out)
        inception_6a_relu_3x3_reduce_out = self.inception_6a_relu_3x3_reduce(inception_6a_3x3_reduce_bn_out)
        inception_6a_3x3_out = self.inception_6a_3x3(inception_6a_relu_3x3_reduce_out)
        inception_6a_3x3_bn_out = self.inception_6a_3x3_bn(inception_6a_3x3_out)
        inception_6a_relu_3x3_out = self.inception_6a_relu_3x3(inception_6a_3x3_bn_out)
        inception_6a_double_3x3_reduce_out = self.inception_6a_double_3x3_reduce(depth)
        inception_6a_double_3x3_reduce_bn_out = self.inception_6a_double_3x3_reduce_bn(inception_6a_double_3x3_reduce_out)
        inception_6a_relu_double_3x3_reduce_out = self.inception_6a_relu_double_3x3_reduce(inception_6a_double_3x3_reduce_bn_out)
        inception_6a_double_3x3_1_out = self.inception_6a_double_3x3_1(inception_6a_relu_double_3x3_reduce_out)
        inception_6a_double_3x3_1_bn_out = self.inception_6a_double_3x3_1_bn(inception_6a_double_3x3_1_out)
        inception_6a_relu_double_3x3_1_out = self.inception_6a_relu_double_3x3_1(inception_6a_double_3x3_1_bn_out)
        inception_6a_double_3x3_2_out = self.inception_6a_double_3x3_2(inception_6a_relu_double_3x3_1_out)
        inception_6a_double_3x3_2_bn_out = self.inception_6a_double_3x3_2_bn(inception_6a_double_3x3_2_out)
        inception_6a_relu_double_3x3_2_out = self.inception_6a_relu_double_3x3_2(inception_6a_double_3x3_2_bn_out)
        inception_6a_pool_out = self.inception_6a_pool(depth)
        inception_6a_pool_proj_out = self.inception_6a_pool_proj(inception_6a_pool_out)
        inception_6a_pool_proj_bn_out = self.inception_6a_pool_proj_bn(inception_6a_pool_proj_out)
        inception_6a_relu_pool_proj_out = self.inception_6a_relu_pool_proj(inception_6a_pool_proj_bn_out)
        inception_6a_output_out = torch.cat([inception_6a_relu_1x1_out,inception_6a_relu_3x3_out,inception_6a_relu_double_3x3_2_out,inception_6a_relu_pool_proj_out], 1)
        depth = self.inception_6e_pool(inception_6a_output_out)        
       
        
                       
         
        x = self.att_d_layer4(depth) + self.att_rgb_layer4(x)   
...
def resnet50_ours(pretrained=False, progress=True, **kwargs):
