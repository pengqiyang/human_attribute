...
__all__ = ['resnet_attention_depth_spatial']
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.theta_0 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=1)
        self.phi_0 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=1)        
        self.theta_1 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=1)
        self.phi_1 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=1)
        self.theta_2 = nn.Conv2d(in_channels=128, out_channels=128, kernel_size=1)
        self.phi_2 = nn.Conv2d(in_channels=128, out_channels=128, kernel_size=1)
        #self.theta_3 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=1)
        #self.phi_3 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=1)
        #self.theta_4 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=1)
        #self.phi_4 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=1)
    ...
    def forward(self, x, depth):
        ...
        x = self.att_d(depth) + self.att_rgb(x) #merge
        theta_x = self.theta_0(x).view(batch_size, 64, -1)
        phi_x = self.phi_0(x).view(batch_size, 64, -1)
        theta_x = theta_x.permute(0, 2, 1)
        f = torch.matmul(theta_x, phi_x)     
        f_div_C = F.softmax(f, dim=-1)
        y = torch.matmul(f_div_C, x.view(batch_size, 64, -1).permute(0,2,1))
        y = y.permute(0, 2, 1).contiguous()
        y = y.view(batch_size, 64, x.size()[2], x.size()[3])
        output_0 = x
        x=x+y
        output_1 = x
        ...
        theta_x = self.theta_1(x).view(batch_size, 64, -1)
        phi_x = self.phi_1(x).view(batch_size, 64, -1)
        theta_x = theta_x.permute(0, 2, 1)
        f = torch.matmul(theta_x, phi_x)     
        f_div_C = F.softmax(f, dim=-1)
        y = torch.matmul(f_div_C, x.view(batch_size, 64, -1).permute(0,2,1))
        ...
        theta_x = self.theta_2(x).view(batch_size, 128, -1)
        phi_x = self.phi_2(x).view(batch_size, 128, -1)
        theta_x = theta_x.permute(0, 2, 1)
        f = torch.matmul(theta_x, phi_x)     
        f_div_C = F.softmax(f, dim=-1)
        y = torch.matmul(f_div_C, x.view(batch_size, 128, -1).permute(0,2,1))
        y = y.permute(0, 2, 1).contiguous()
        y = y.view(batch_size, 128, x.size()[2], x.size()[3])
        output_4 = x
        x=x+y
        output_5 = x
        ...
        return x, output_0, output_1, output_2,output_3, output_4, output_5
...
def resnet_attention_depth_spatial(pretrained=False, progress=True, **kwargs):
